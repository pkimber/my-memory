...
CompRackspace = libcloud.compute.providers.get_driver(
    libcloud.compute.types.Provider.RACKSPACE_NOVA_DFW
)

# Use the compute driver to create a connection
compdriver = CompRackspace(
    config["username"],
    config["apikey"],
...
install_mongo = ScriptDeployment(open(os.path.expanduser("~/ztdeploy/dbdeploy.sh")))
...
def mycreatenode(servername, image, size, deploy):
    ...
    t_compdriver.deploy_node(name=servername, image=image, size=size, deploy=deploy)
    print(
        "Finished creating %s with %s and size %s" % (servername, image.name, size.name)
    )
...
jobs.append(
    gevent.spawn(mycreatenode, "db-zt01.gekitsuu.org", image, dbsize, db_deploy_steps)
)
...
new_balancer = lbdriver.create_balancer(
    name="zombietracker-lb",
